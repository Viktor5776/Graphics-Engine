...
namespace Hydro::gfx::Bind
{
	...
	void RenderTarget::BindAsBuffer( Graphics& gfx, DepthStencil* depthStencil ) noexcept(!_DEBUG)
	{
		BindAsBuffer( gfx, depthStencil ? depthStencil->pDepthStencilView.Get() : nullptr );
	}

	void RenderTarget::BindAsBuffer( Graphics& gfx, ID3D11DepthStencilView* pDepthStencilView ) noexcept(!_DEBUG)
	{
	...
	void RenderTarget::Clear( Graphics& gfx, const std::array<float, 4>& color ) noexcept(!_DEBUG)
	{
	...
	misc::Surface Bind::ShaderInputRenderTarget::ToSurface( Graphics& gfx ) const
	{
		HRESULT hr;
		namespace wrl = Microsoft::WRL;
		...
		{
			auto pSrcRow = reinterpret_cast<const misc::Surface::Color*>(pSrcBytes + msr.RowPitch * size_t( y ));
	...
	void ShaderInputRenderTarget::Bind( Graphics& gfx ) noexcept(!_DEBUG)
	{
		GetContext( gfx )->PSSetShaderResources( slot, 1, pShaderResourceView.GetAddressOf() );
	}


	void OutputOnlyRenderTarget::Bind( Graphics& gfx ) noexcept(!_DEBUG)
	{
