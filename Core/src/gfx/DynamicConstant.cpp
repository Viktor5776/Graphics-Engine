...
#undef X
	...
	std::pair<size_t, const LayoutElement*> LayoutElement::CalculateIndexingOffset( size_t offset, size_t index ) const noexcept(!_DEBUG)
	{
	...
	size_t LayoutElement::GetOffsetBegin() const noexcept(!_DEBUG)
	{
		return *offset;
	}
	size_t LayoutElement::GetOffsetEnd() const noexcept(!_DEBUG)
	{
		switch( type )
		{
...
#undef X
	...
	size_t LayoutElement::GetSizeInBytes() const noexcept(!_DEBUG)
	{
		return GetOffsetEnd() - GetOffsetBegin();
	}
	LayoutElement& LayoutElement::Add( Type addedType, std::string name ) noexcept(!_DEBUG)
	{
	...
	LayoutElement& LayoutElement::Set( Type addedType, size_t size ) noexcept(!_DEBUG)
	{
	...
	LayoutElement::LayoutElement( Type typeIn ) noexcept(!_DEBUG)
		:
	type{ typeIn }
	{
		assert( typeIn != Empty );
...
#undef X
	...
	std::string LayoutElement::GetSignatureForStruct() const noexcept(!_DEBUG)
	{
	...
	std::string LayoutElement::GetSignatureForArray() const noexcept(!_DEBUG)
	{
		using namespace std::string_literals;
		const auto& data = static_cast<ExtraData::Array&>(*pExtraData);
	...
	std::string Layout::GetSignature() const noexcept(!_DEBUG)
	{
	...
	{}
	LayoutElement& RawLayout::operator[]( const std::string& key ) noexcept(!_DEBUG)
	{
		return (*pRoot)[key];
	}
	...
	ElementRef ElementRef::operator[]( const std::string& key ) const noexcept(!_DEBUG)
	{
		return { &(*pLayout)[key],pBytes,offset };
	}
	ElementRef ElementRef::operator[]( size_t index ) const noexcept(!_DEBUG)
	{
		const auto indexingData = pLayout->CalculateIndexingOffset( offset, index );
		return { indexingData.second,pBytes,indexingData.first };
	}
	...
	Buffer::Buffer( RawLayout&& lay ) noexcept(!_DEBUG)
		:
	Buffer( LayoutCodex::Resolve( std::move( lay ) ) )
	{}
	Buffer::Buffer( const CookedLayout& lay ) noexcept(!_DEBUG)
		:
		pLayoutRoot( lay.ShareRoot() ),
		bytes( pLayoutRoot->GetOffsetEnd() )
	{}
	Buffer::Buffer( CookedLayout&& lay ) noexcept(!_DEBUG)
		:
	...
	void Buffer::CopyFrom( const Buffer& other ) noexcept(!_DEBUG)
	{
