...
#undef X
	...
	public:
		...
		std::pair<size_t, const LayoutElement*> CalculateIndexingOffset( size_t offset, size_t index ) const noexcept(!_DEBUG);
		...
		size_t GetOffsetBegin() const noexcept(!_DEBUG);
		size_t GetOffsetEnd() const noexcept(!_DEBUG);
		// get size in bytes derived from offsets
		size_t GetSizeInBytes() const noexcept(!_DEBUG);
		// only works for Structs; add LayoutElement to struct
		LayoutElement& Add( Type addedType, std::string name ) noexcept(!_DEBUG);
		template<Type typeAdded>
		LayoutElement& Add( std::string key ) noexcept(!_DEBUG)
		{
			return Add( typeAdded, std::move( key ) );
		}
		// only works for Arrays; set the type and the # of elements
		LayoutElement& Set( Type addedType, size_t size ) noexcept(!_DEBUG);
		template<Type typeAdded>
...
#undef X
	...
	private:
		...
		LayoutElement( Type typeIn ) noexcept(!_DEBUG);
		...
		std::string GetSignatureForStruct() const noexcept(!_DEBUG);
		std::string GetSignatureForArray() const noexcept(!_DEBUG);
	...
	public:
		...
		bool Exists() const noexcept;
		ElementRef operator[]( const std::string& key ) const noexcept(!_DEBUG);
		ElementRef operator[]( size_t index ) const noexcept(!_DEBUG);
		// optionally set value if not an empty Ref
		template<typename S>
		bool SetIfExists( const S& val ) noexcept(!_DEBUG)
		{
			if( Exists() )
			{
				*this = val;
	...
	public:
		...
		Buffer( RawLayout&& lay ) noexcept(!_DEBUG);
		Buffer( const CookedLayout& lay ) noexcept(!_DEBUG);
		Buffer( CookedLayout&& lay ) noexcept(!_DEBUG);
		...
		void CopyFrom( const Buffer& ) noexcept(!_DEBUG);
