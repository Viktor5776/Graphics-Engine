...
namespace Hydro::misc
{
		...
		note( "[Note] " + note )
	{
		if( hr )
		{
			note = "[Error String] " + win::WindowException::TranslateErrorCode( *hr ) + "\n" + note;
		}
	}
		...
		HydroException( line, file )
	{
		...
		if( hr )
		{
			note = "[Error String] " + win::WindowException::TranslateErrorCode( *hr ) + note;
		}
	}
	...
	const char* Surface::Exception::what() const noexcept
	{
		...
		oss << HydroException::what() << std::endl << GetNote();
		whatBuffer = oss.str();
